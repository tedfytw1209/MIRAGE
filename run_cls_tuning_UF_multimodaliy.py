...
"""True multi-modal (OCT B-scan + SLO) finetuning/evaluation of MIRAGE on
the UF cohort. Unlike `run_cls_tuning_UF.py` (which picks a single modality
per run), this script feeds both the middle OCT B-scan slice and the SLO
scan of each sample to MIRAGE at once: `mirage_wrapper.MIRAGEClsGlobal`
concatenates both domains' tokens and jointly attends to them through the
shared encoder before pooling (see `mirage_wrapper.py` for the dict-input
path added to support this).
"""
...
def get_args():
    ...
    # wandb
    parser.add_argument(
        '--wandb_project', default='MIRAGE_UF_result_mm', type=str,
...
def process_args(args):
    ...
    # Fixed: this script always feeds both domains at once, unlike
    #   run_cls_tuning_UF.py's single, user-selectable `--uf_modality`.
    args.uf_modality = 'bscan-slo'
...
def build_dataset(subset, args, build_transform: Callable, augment=False):
    ...
    dataset = UFCohortMultiModalDataset(
        csv_file=csv_file,
        root_dir=args.data_root,
        split=subset,
        transform_bscan=transform,
        transform_slo=transform,
    )
    return dataset
